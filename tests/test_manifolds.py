...
class TestManifolds:
    def test_manifolds(self):
        ...
        cisd_sym_manifold = []
        for i in range(4):
            cisd_sym_manifold += manifolds.cisd_manifold(ref, irreps = irreps, target_irrep = i) 
        ...
        h2p_manifold = manifolds.ee_ip_ea_manifold(ref, 2, 1, sz = [-1.5, -.5, .5, 1.5])
        assert len(h2p_manifold) == 90

        h2p_sym_manifold = []
        for i in range(4):
            h2p_sym_manifold += manifolds.ee_ip_ea_manifold(ref, 2, 1, sz = [-1.5, -.5, .5, 1.5], irreps = irreps, target_irrep = i)
        assert len(h2p_sym_manifold) == 90
            
        p2h_manifold = manifolds.ee_ip_ea_manifold(ref, 1, 2, sz = [-1.5, -.5, .5, 1.5])
        assert len(p2h_manifold) == 90

        hp2_sym_manifold = []
        for i in range(4):
            hp2_sym_manifold += manifolds.ee_ip_ea_manifold(ref, 1, 2, sz = [-1.5, -.5, .5, 1.5], irreps = irreps, target_irrep = i)
