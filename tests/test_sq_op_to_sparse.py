...
class TestOpToSparse:
    def test_sq_op_to_scipy(self):
        ...
        for i in [-2.5, -1.5, -.5, 0, .5, 1.5, 2.5]:
            for j in range(0, 8):
                H_pen = H + 1000*np.square((Sz - i*np.eye(hdim))) + 1000*np.square((N - j*np.eye(hdim)))
                w, v = np.linalg.eigh(H_pen)
                H_sym = sq_op_to_scipy(mol.sq_hamiltonian, N_qubits, N = j, Sz = i).todense()
