...
class QITE_FCI(Algorithm):
    ...
    def run(self,
        ...
        # new stuff I added for FCI
        self._sz = 0
        # self._dim = 2 * self._norb
        # self._max_nbody = 2 # I have very little faith this is right
        # self.Top = qf.TensorOperator(self._max_nbody, self._dim)

        if(computer_type=='fci'):
            qc_ref = qf.FCIComputer(self._nel, self._sz, self._norb) # ask Nick if sz, which I think is sum of individual quantum spin numbers, is always zero for our claculations
            qc_ref.hartree_fock() # is this necessary?

            if(apply_ham_as_tensor):
                qc_ref.apply_tensor_spat_012bdy(
                ...
                qc_ref.apply_sqop(self._sq_ham)
        ...
        # I am not going to worry abou adding FCI Computer updates to this yet
    ...
    def build_expansion_pool(self):
        ...
        if(computer_type=='fci'):
            ...
            if(self._expansion_type in {'SD', 'GSD', 'SDT', 'SDTQ', 'SDTQP', 'SDTQPH'}):
                ...
                P.fill_pool(self._expansion_type) # This automatically filters non-particle conserving terms, the codebelow is unnecessary!

                # FROM MOTTA:
                # For a number conserving Hamiltonian, such as the
                # fermionic Hubbard Hamiltonian treated in Fig. 3 in the main
                # text, we retain only those terms with an equal number of cre-
                # ation and annihilation operators, to conserve particle number.
                for _, sqop in P.terms(): # ignores the coefficient of each sqop in pool
                    temp_sqop = qf.SQOperator()
                    for term in sqop.terms():
                        if len(term[1]) == len(term[2]): # checks to see if term contins equal number of creation and annahilation operators
                            temp_sqop.add_term(term[0], term[1], term[2])

                    self._sig.add(1.0, temp_sqop)
    ...
    def build_S_FCI(self):
        ...
        Ipsi_qc.set_state(copy.deepcopy(self._qc))
        # CI[I][J] = (σ_I Ψ)_J
        ...
        for i in range(Idim):
            ...
            CI[i,:] = copy.deepcopy(Ipsi_qc)
            for j in range(i):
                S[i][j] = S[j][i] = np.vdot(CI[i,:], CI[j,:])
            Ipsi_qc.set_coeff_vec(copy.deepcopy(self._qc))
    ...
    def build_sparse_S_b_FCI(self, b):
        b_sparse = []
        idx_sparse = []
