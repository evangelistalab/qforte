...
def ucc_ph_energy_opt(mol, ref, order = 2, guess_source='zeros', include_zero_amps=False, maxiter=100):
    ...
    if(guess_source == 'ccsd'):
        raise ValueError('using ccsd initial amplitude guess for arbitrary order UCC is not yet supported.')
    ...
    if(guess_source == 'zeros'):
        # NOTE: need to change the name of 'singlet' to just be the general one...
        T_sq = ucc_helpers.get_singlet_ucc_zeros_lists(nocc, nvir, order = order, make_anti_herm=False)
