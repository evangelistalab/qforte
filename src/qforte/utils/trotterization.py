...
def trotterize(operator, trotter_number=1, trotter_order=1):
    """
    A function which returns an exponentiated version of
    a parameterized qubit state preparation circuit or operator.

    Usage

    :param operator: the operator or state preparation ansatz
    (represented as a sum of pauli terms) to be exponentiated

    :param trotter_number: for an operator A with terms A_i, the trotter_number
    is the exponent (N) for to product of single term
    exponentals e^A ~ ( Product_i(e^(A_i/N)) )^N

    :param trotter_order: the order of the troterization approximation, can be 1
    or 2
    """
